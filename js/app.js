...
document.documentElement.addEventListener("mousedown", () => {
  Tone.context.state !== "running" && Tone.context.resume();
});
/**
* MusicalScale
...
class MusicalScale {
  ...
  _loadScale(params) {
    ...
    for (let s = 0; s < this._scale.steps.length; s++) {
      ...
      let triad = this._genTriad(
        s,
        idx,
        rel_octave,
        this._scale.triads[s]
      );
      // define the note
      let note = {
        step: s,
  ...
  _genTriad(s, offset, octave, t) {
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      let rel_octave =
        offset + step > keys.length - 1 ? octave + 1 : octave;
      // define the note
...
class ArpPlayer {
  constructor(params) {
    ...
    this._loadSynths();
    this._loadTransport(); // change tabs, pause player
  ...
  _loadSynths() {
    ...
    this.synths.bass.voice1.envelope.attack = 0.05; // fx mixes
    this.fx.distortion.wet.value = 0.2;
    this.fx.reverb.wet.value = 0.2;
    this.fx.delay.wet.value = 0.3; // gain levels
    this.channel.master.toMaster();
    this.channel.treb.connect(this.channel.master);
    this.channel.bass.connect(this.channel.master); // fx chains
    this.synths.treb.chain(
      this.fx.delay,
      this.fx.reverb,
      this.channel.treb
    );
  ...
  _loadTransport() {
    ...
    Tone.Transport.scheduleRepeat((time) => {
      ...
      let chord = this.MS.notes[this.chords[curr_chord]]; // finding the current note
      ...
      let note =
        notes[this.arpeggio[this.player.step % this.arpeggio.length]]; // setting bass notes
      let bass_o = chord.rel_octave + 2;
      let bass_1 = chord.note + bass_o; // slappin da bass
      ...
      if (
        this.player.step %
        (this.arpeggio.length * this.player.arp_repeat) ===
        0
      ) {
  ...
  _updateOutput() {
    ...
    this.chords.forEach((chord) => {
      ...
      el.innerHTML = `${note.note.replace("#", "<sup>♯</sup>")} <small>${note.triad.type
        }</small>`;
  ...
  _updatePatternSelector() {
    this.pattern_container.innerHTML = ""; // reset if the id is over
    this.ap_pattern_id =
      this.ap_pattern_id >
        this.AP.patterns[this.ap_pattern_type].length - 1
        ? 0
        : this.ap_pattern_id;
    this.arpeggio =
